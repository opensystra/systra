...
def read_shp(filename,  encoding=None, type=False):
...
def write_shp(
    ...
    try:
        ...
        geometry = tqdm(list(df.pop('geometry')), 'write geometry')
    ...
    iterator = iterlist
    #iterator = tqdm(iterlist, 'write records')
    ...
    if re_write or encoding:
        pandasdbf.write_dbf(
            df[list(set(df.columns) - {'geometry'})],
            filename + 'dbf',
            encoding=encoding
        )
    ...
    if projection_string:
        with open(filename.split('.')[0] + '.prj', 'w') as prj_file:
            prj_file.write(projection_string)
    if style_file:
        shutil.copyfile(style_file, filename.split('.')[0] + '.qml')

    if projection_file:
        shutil.copyfile(projection_file, filename.split('.')[0] + '.prj')

    if shp_file:
        shutil.copyfile(shp_file, filename.split('.')[0] + '.shp')

    if shx_file:
        shutil.copyfile(shx_file, filename.split('.')[0] + '.shx')
...
def od_matrix(zones, centroids=False):
    ...
    iterate = [pos.index]*2
    od = pd.DataFrame(index=pd.MultiIndex.from_product(iterate, names=['origin', 'destination'])).reset_index()  #: the od column matrix
